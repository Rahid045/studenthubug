...
?>
<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Dashboard | Student hub ug</title><link rel="stylesheet" href="style.css"></head><body>
<header class="site-header"><div class="container nav"><a class="logo" href="index.html"><span class="logo-mark">S</span>Student hub ug</a><ul class="nav-links"><li><a href="resources.php">Resources</a></li><?php if(($user['role'] ?? 'student') === 'admin'):?><li><a href="upload.php">Upload</a></li><?php endif;?><li><a href="tutoring.php">Tutoring</a></li><li><a class="active" href="dashboard.php">Dashboard</a></li></ul><div class="nav-actions"><a class="btn btn-danger" href="logout.php">Log out</a></div></div></header>
...
<section class="dashboard-stats"><?php if(($user['role'] ?? 'student') === 'admin'):?><article class="dashboard-card"><small>Uploaded materials</small><div class="number"><?=$uploads?></div><a href="upload.php">Upload a resource →</a></article><?php endif;?><article class="dashboard-card"><small>Tutoring requests</small><div class="number"><?=$requests?></div><a href="tutoring.php">Request tutoring →</a></article><article class="dashboard-card"><small>Account status</small><div class="number">✓</div><small>Active <?=htmlspecialchars($user['role']??'student')?> account</small></article></section>
<?php if(($user['role'] ?? 'student') === 'admin'):?><section class="dashboard-section"><h2>Your recent uploads</h2><?php if(!$my):?><div class="empty">No materials uploaded yet. <a href="upload.php">Upload your first resource</a>.</div><?php else:?><div class="table-wrap"><table><thead><tr><th>Resource</th><th>Course unit</th><th>Type</th><th>Date</th></tr></thead><tbody><?php foreach($my as $r):?><tr><td><strong><?=htmlspecialchars($r['title'])?></strong></td><td><?=htmlspecialchars($r['course_unit']??'—')?></td><td><span class="badge"><?=htmlspecialchars($r['resource_type'])?></span></td><td><?=htmlspecialchars(date('d M Y',strtotime($r['uploaded_at'])))?></td></tr><?php endforeach;?></tbody></table></div><?php endif;?></section><?php endif;?>
